...
typedef struct
{
    ...
    double observation[MAX_STATE][MAX_OBSERV]; // observation prob.
} HMM;
    ...
    while (fscanf(fp, "%s", token) > 0)
    {
        ...
        else if (strcmp(token, "observation:") == 0)
        {
            ...
            for (i = 0; i < hmm->observ_num; i++)
                for (j = 0; j < hmm->state_num; j++)
                    n = fscanf(fp, "%lf", &(hmm->observation[j][i]));
        }
    }
}
...
static void dumpHMM(FILE *fp, HMM *hmm)
{
    ...
    {
        for (j = 0; j < hmm->state_num - 1; j++)
            fprintf(fp, "%.5lf ", hmm->observation[j][i]);
        fprintf(fp, "%.5lf\n", hmm->observation[hmm->state_num - 1][i]);
    }
}
